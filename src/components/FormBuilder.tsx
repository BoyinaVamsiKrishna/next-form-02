...
const [formBackground, setFormBackground] = useState<FormBackgroundState>({
  ...
  const handlePublishBtn: PublishFormBtnProps = (elementsArrayData: FormElement[]) => {
    ...
    const handleBackgroundChange = useCallback((newBackground: any) => {
      setFormBackground(newBackground);
    }, []);
  ...
  useEffect(() => {
    ...
    console.log("Use Effect from Form Builder Component")
    console.log(formBackground)
  }, [updatedElements, updatedelementsArray,formBackground]);
    ...
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <main className="flex flex-col w-full min-h-screen">
        <nav className="flex justify-between border-b-2 p-4 gap-3 items-center">
          ...
          <PreviewDialogBtn formBackground={formBackground} />
            {!form?.published && (
              <>
        ...
        <DynamicBackground>
        <Designer onBackgroundChange={handleBackgroundChange} />
        </DynamicBackground>
      </main>
      <DragOverlayWrapper />
    </DndContext>
